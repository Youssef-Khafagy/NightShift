...
3. **Code done 2026-09-23, waiting for the owner's review (widens permissions).** Ten tools in `agent/tools/` (26 tests, fake clients), all ten schemas accepted live by Groq, Gemini and Mistral (tool definitions cost about 900 to 1,300 input tokens per call). `terraform/investigator.tf`: role, inline policy, boundary; CI apply role gains managed-policy and boundary actions on `nightshift-*`. Planned policies checked with `simulate-custom-policy` before apply: 29 of 29 as expected. Found and fixed an answer-key leak: chaos rollback reasons named the scenario in the deployments table; now a neutral constant held to the banned-words test. Six existing rows still carry the old text. Plan: **Read-only tools and the Investigator role.** The ten tools, output capped and summarised, marked untrusted. Terraform role with a permissions boundary and explicit denies on IAM, Terraform state and all writes, verified with `simulate-principal-policy`. Locally the agent assumes this role. **Owner review point.**
