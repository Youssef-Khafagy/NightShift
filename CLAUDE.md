...
7. **In progress (2026-09-21).** Measure DPU per checkout and replace the estimate in COST.md before any load test.
   ...
   - **Still to do:** deploy the fix, then run `scripts/measure_dpu.py --batch 5 --batch 25` and rebuild the projection. The `~20K DPUs` estimate stays marked weak until then.
