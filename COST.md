...
**Resolved 2026-09-22: Lambda logs count against the 5 GB free tier.** Since May 2025 Lambda logs can be billed as vended logs (usage type `VendedLog-Bytes`, tiered from $0.50/GB in US East), and no official page found says whether the free tier covers that usage type. The account's own bill answered it. The September 2026 bill for CloudWatch in Canada (Central) lists exactly three line items: API requests (66, free tier), `CAN1-TimedStorage-ByteHrs` ("First 5GB-mo per month of logs storage is free", 0 GB-Mo) and `PutLogEvents` ("First 5GB per month of log data ingested is free", 0 GB), all $0.00, with no vended logs line. Lambda is the only thing in this account that writes logs, and a line item appears even when usage rounds to 0 GB, so Lambda ingestion is being recorded as free tier `PutLogEvents`. The Free Tier API agrees: 228,970 bytes of `Global-DataProcessing-Bytes` this month, against 381,085 bytes of `IncomingBytes`; the gap is not explained and is small enough not to matter. If this ever changes, the worst case is about 0.42 GB x $0.50 = ~$0.21 in a benchmark month, and the $0.01 tripwire budget fires after roughly 20 MB of paid ingestion. The monthly check reads the bill's CloudWatch line items for a vended logs entry.
